...
- **Audit pop-up.** Opens once per session when the visitor reaches the end of the page (the closing CTA band scrolls into view), never earlier; the top-hero field opens the same form on demand. Code: the audit block in `site.js`.
- **Top hero.** `index.html` opens with a centred headline, one website field and floating business owners (`OWNERS` + `renderOrbit` in `site.js`; styles under "00 Top hero" in `styles.css`). The faces are Unsplash portraits (Unsplash Licence, free for commercial use) loaded from Unsplash's CDN, with an illustrated fallback (`avatarSvg`) if a photo fails. To use your own clients' photos with their permission, put files in `assets/owners/` and set `photo` to the path. Labels name business types only; the process is explained in the section below.
- **Backdrop.** A seamless jigsaw of near-black pieces plus the amber glow sits on `body::before`, fixed to the viewport, on every page. The jigsaw is an inline SVG tile in the `--puzzle` token (dark and light versions) in `styles.css`; regenerate it with a small script if you want other shades or a different piece size (two pieces per 320px tile; the tile edge runs through piece centres so it repeats without seams). Glow strength: `--glow`, `--glow-2`. There are no divider lines between sections. Submitting opens the audit modal with the website pre-filled (`DS.openAudit`). The original hero with the 3D board follows as the second screen.
