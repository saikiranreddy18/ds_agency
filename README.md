...
- **Top hero.** `index.html` opens with a centred headline, one website field and the floating outcome bubbles (`renderOrbit` in `site.js`, pool and slots at the top of that function; styles under "00 Top hero" in `styles.css`). Submitting opens the audit modal with the website pre-filled (`DS.openAudit`). The original hero with the 3D board follows as the second screen.
