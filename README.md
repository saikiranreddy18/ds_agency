...
- **Backdrop.** The faint grid and amber glow sit on `body::before`, fixed to the viewport, on every page. Tune `--grid-line`, `--glow` and `--glow-2` in the token block of `styles.css` (dark and light values). Submitting opens the audit modal with the website pre-filled (`DS.openAudit`). The original hero with the 3D board follows as the second screen.
