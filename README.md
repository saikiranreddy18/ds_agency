...
- **Top hero.** `index.html` opens with a centred headline, one website field and floating business owners (illustrated avatars, `OWNERS` + `avatarSvg` + `renderOrbit` in `site.js`; styles under "00 Top hero" in `styles.css`). They name business types only; the process is explained in the section below, so nothing up top repeats it. Submitting opens the audit modal with the website pre-filled (`DS.openAudit`). The original hero with the 3D board follows as the second screen.
