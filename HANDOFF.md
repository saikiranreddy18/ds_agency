...
- **Backup inbox:** if the webhook and the relay both fail, the visitor's browser emails the same fields through FormSubmit to `formBackupEmail` in `site.config.js` (`saikiranreddytallapureddy@gmail.com`; subject ends with "(backup channel)"). FormSubmit sent that inbox a one-time "Activate Form" email on 2026-09-03; click it once, otherwise backup deliveries are held. The relay tries the same backup server-side too, but FormSubmit's bot protection usually blocks calls from Vercel, so the browser-side path is the one that counts.
